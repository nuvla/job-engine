...
@distribution('register_usage_record_updated_deployment')
class RegisterUsageRecordUpdatedDeploymentJobsDistribution(DistributionBase):
    DISTRIBUTION_NAME = 'register_usage_record_updated_deployment'

    def __init__(self, distributor):
        super(RegisterUsageRecordUpdatedDeploymentJobsDistribution, self).__init__(
    ...
    def job_generator(self):
        deployments = self.updated_deployments()
        logging.info(f'Updated deployments count: {len(deployments)}')
        owners = self.unique_owners(deployments)
        logging.info(f'Updated deployments owners count: {len(owners)}')
        for owner in owners:
