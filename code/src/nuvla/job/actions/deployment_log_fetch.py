...
class DeploymentLogFetchJob(object):
    ...
    def fetch_log(self, resource_log):
        ...
        service_name = resource_log['components'][0]

        last_timestamp = resource_log.get('last-timestamp')

        since = resource_log.get('since')

        lines = resource_log.get('lines', 200)

        deployment_id = resource_log['parent']
        ...
        if Deployment.is_application_kubernetes(deployment):
            ...
            list_opts = [service_name, '--timestamps=true', '--tail',
                         str(lines),
        ...
        update_deployment_log = {'log': {service_name: result}}

        if new_last_timestamp:
            update_deployment_log['last-timestamp'] = new_last_timestamp

        self.api.edit(resource_log['id'], update_deployment_log)

    def fetch_deployment_log(self):
        resource_log_id = self.job['target-resource']['href']

        log.info('Job started for {}.'.format(resource_log_id))

        resource_log = self.api.get(resource_log_id,
                                    select='id, parent, components, since, line'
                                           ', last-timestamp').data
        ...
        try:
            ...
            try:
                ...
                log.error('Failed to set error state for {0}: {1}'
                          .format(resource_log_id, ex_state))
