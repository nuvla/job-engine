...
def create_connector_instance(api_infrastructure_service, api_credential, api_endpoint):
    ...
    if not hasattr(connector, 'instantiate_from_cimi'):
        raise NotImplementedError('The infrastructure service "{}" is not compatible with the start_deployment job'
                                  .format(api_infrastructure_service['type']))
    return connector.instantiate_from_cimi(api_infrastructure_service, api_credential, api_endpoint)